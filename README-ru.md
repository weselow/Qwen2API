...
QWEN_CHAT_ENDPOINT_IPS=       # Закрепить адреса точки входа chat.qwen.ai через запятую (например: 47.254.175.31,47.91.78.155)
QWEN_CLI_ENDPOINT_IPS=        # То же для portal.qwen.ai
...
| `QWEN_CHAT_ENDPOINT_IPS` | Закреплённые адреса точки входа `chat.qwen.ai` через запятую. Alibaba выдаёт адрес по местоположению спрашивающего, и в некоторых странах достаётся точка, которая устанавливает шифрованное соединение, но не передаёт данные. При работе через прокси имя разрешает сам прокси, поэтому запись в `hosts` не помогает. Если список задан — соединение идёт по этим адресам, а имя остаётся в приветствии TLS, в проверке сертификата и в заголовке `Host` (то же, что `curl --connect-to`); проверка сертификата не отключается, при отказе адреса служба берёт следующий. Пусто — поведение прежнее | `47.254.175.31,47.91.78.155` |
| `QWEN_CLI_ENDPOINT_IPS` | То же для `portal.qwen.ai` (канал CLI) | `47.254.175.31` |
